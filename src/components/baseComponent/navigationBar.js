...
export default class NavigationBar extends Component {
  ...
  renderLeftItem() {
    ...
    if (this.navigationBarProps.leftIcon) {
      ...
      tempComponent = (
        ...
        // <Image style={{width: icon.width, height: icon.height, color: icon.color}}></Image>
        <Image style={{width: icon.width, height: icon.height, tintColor: icon.color}} source={require('../../assets/image/back.png')}></Image>

        // <Text style={{backgroundColor:'red'}}>aaaa</Text>
      )
      ...
      tempComponent = (
        ...
        <Image style={{width: 50, height: 50, tintColor: commonStyle.iconGray, resizeMode:'center', marginBottom:-40}} source={require('../../assets/image/back.png')}></Image>

        // <Text style={{width: 50, height: 50}} style={{backgroundColor:'red'}}>bbb</Text>
      )
    }
    return (
      <TouchableOpacity
  ...
  renderRightItem() {
    ...
    if (this.navigationBarProps.rightIcon) {
      ...
      tempComponent = (
        // <Icon name={icon.name} size={icon.size} color={icon.color}/>
        // <Image style={{width: icon.width, height: icon.height}, source={{icon.name}}}></Image>
        <Text style={{backgroundColor:'green', width:50, height: 50,marginBottom:-44,display:'flex',flexDirection:'row',alignContent:'center',textAlign:'center', justifyContent:'center'}}>cccc112</Text>
      )
...
const styles = StyleSheet.create({
  navBarStyle: {
    ...
    borderBottomColor: commonStyle.backGroundColor,
  },
  navContentStyle: {
    flexDirection: 'row',
    alignItems: 'center',
